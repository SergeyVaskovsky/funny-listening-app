...
export const ElementUpdate = () => {
  ...
  const saveEntity = values => {
    const entity = {
      ...elementEntity,
      ...values,
      entireStoryElement: stories.find(it => it.id.toString() === values.entireStoryElement.toString()),
      element: stories.find(it => it.id.toString() === values.element.toString()),
    };
  ...
  const defaultValues = () =>
    isNew
      ...
      : {
          ...elementEntity,
          entireStoryElement: elementEntity?.entireStoryElement?.id,
          element: elementEntity?.element?.id,
        };
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField id="element-element" name="element" data-cy="element" label="Element" type="select">
                <option value="" key="0" />
                {stories
                  ? stories.map(otherEntity => (
                      <option value={otherEntity.id} key={otherEntity.id}>
